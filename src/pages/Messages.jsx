...
function Messages() {
      ...
      const fetchUserDetails = async (userId) => {
        ...
        onValue(userDetailsRef, (snapshot) => {
          try {
            ...
            if (userDetailsData && !userDetailsData.isDeleted) {
              setUserDetails((prevUserDetails) => ({
                ...prevUserDetails,
                [userId]: userDetailsData,
              }));
            } else if (userDetailsData && userDetailsData.isDeleted) {
              // If user is marked as deleted, hide the card
  ...
  const handleDeleteUser = () => {
    ...
    // Hide the card of the deleted user
    setDeletedUsers((prevDeletedUsers) => [...prevDeletedUsers, userIdToDelete]);
  };
      ...
      <div className="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
        ...
        <div className="flex">
          <div className="w-1/3 p-4 border-r">
            ...
            <ul>
              {userIds.map((userId) => {
                const userDeleted = deletedUsers.includes(userId);
                const userInboxLength = userInbox.filter((message) => message.userId === userId).length;

                return !userDeleted || userInboxLength > 0 ? ( // Show if not marked as deleted or has new messages
                  <li
                    key={userId}
              ...
              {userInbox.map((message) => (
                <div
                  ...
                  className={`bg-blue-500 text-white p-4 m-2 ${
                    message.sender ? 'rounded-l-lg' : 'rounded-r-lg'
                  } border`}
                  style={{
                    maxWidth: '80%',
